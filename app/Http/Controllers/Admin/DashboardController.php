...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $stats = [
            'students' => [
                'total' => Student::where('academic_year_id', $currentYear->id)->count(),
                'enrolled' => Student::where('academic_year_id', $currentYear->id)
                    ->whereIn('status', ['enrolled'])
                    ->count(),
                'interested' => Student::where('academic_year_id', $currentYear->id)
                    ->whereIn('status', ['interested', 'prospect'])
                    ->count(),
                'prospect' => Student::where('academic_year_id', $currentYear->id)->where('status', 'prospect')->count(),
            ],
            'enrollments' => [
                'total' => Enrollment::where('academic_year_id', $currentYear->id)
                    ->orWhereHas('student', function($q) use ($currentYear) {
                        $q->where('academic_year_id', $currentYear->id);
                    })
                    ->count(),
                'active' => Enrollment::where(function($q) use ($currentYear) {
                        $q->where('academic_year_id', $currentYear->id)
                          ->orWhereHas('student', function($sq) use ($currentYear) {
                              $sq->where('academic_year_id', $currentYear->id);
                          });
                    })
                    ->where('status', 'active')
                    ->count(),
